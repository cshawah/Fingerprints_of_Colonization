...
colonial_no_na <- colonial_history %>% na.omit()

gdp_plot <- ggplot(colonial_no_na) +
  geom_point(aes(x = ind_date, y = round(as.numeric(gdp_val), 4), color = named_ind_from)) +
  scale_y_log10()

hdi_plot <- ggplot(colonial_history) +
  geom_point(aes(x = ind_date, y = round(hdi_val, 6), color = named_ind_from))

cm_plot <- ggplot(colonial_no_na) +
  geom_point(aes(x = ind_date, y = round(as.numeric(cm_val), 2), color = named_ind_from))
